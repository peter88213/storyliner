#!/usr/bin/python3
"""Install the storyliner application. 
...
[HKEY_CURRENT_USER\Software\Classes\\storyliner]
@="storyliner Project"
[HKEY_CURRENT_USER\Software\Classes\\storyliner\DefaultIcon]
@="$INSTALL\\\\icons\\\\nLogo64.ico"
[HKEY_CURRENT_USER\Software\Classes\\storyliner\shell\open\command]
@="\\"$PYTHON\\" \\"$SCRIPT\\" \\"%1\\""
[HKEY_CURRENT_USER\Software\Classes\\.nvcx]
"Content Type"="text/xml"
@="nv5Collection"
[HKEY_CURRENT_USER\Software\Classes\\nv5Collection]
@="storyliner Collection"
...
PLUGIN_OUTDATED = '''There are outdated plugins installed, which will be ignored by storyliner from now on. 
...
def make_context_menu(installPath):
    """Generate ".reg" files to extend the storyliner context menu."""
    ...
    save_reg_file(f'{installPath}/add_storyliner.reg', Template(ADD_TO_REGISTRY), mapping)
    save_reg_file(f'{installPath}/remove_storyliner.reg', Template(REMOVE_FROM_REGISTRY), {})
...
if __name__ == '__main__':
    ...
    novxlibPath = f'{homePath}/.storyliner'
    try:
        install(novxlibPath)
    ...
    root.openButton = tk.Button(text="Open installation folder", command=lambda: open_folder(f'{homePath}/.storyliner'))
